<template>
  ...
  <div class="grid grid-cols-2 mt-10 p-10"> 
    <div class="max-w-md mx-auto col-span-1">   
      <div class="relative">
    ...
    <div class="max-w-md mx-auto col-span-1">
      <div class="relative">
        <form method="GET" action="/">
          ...
          <input type="hidden" value="productCode" name="type" />
          <input type="search" v-model="pcode" name="search" id="default-search" class="block w-full p-4 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="What are you looking for?" required />
          <button type="submit" class="text-white absolute end-2.5 bottom-2.5 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Search</button>
        </form>
      </div>
      <p>Please input EAN or UDP code. (Eg.5012345678900)</p>
      <p class="text-red-600">{{ !isAsin ? error : '' }}</p>
    </div>
  </div>

  <h2 class="text-2xl mt-5 pl-5 font-extrabold" v-if="summary">Product Summary</h2>
  ...
  <div class="grid grid-cols-3 mt-10 p-10" v-if="title">
    ...
    <div class="col-span-2 pl-10 ml-10">
      <h2 class="text-2xl font-extrabold">{{ title }}</h2> <span class="text-xl text-blue-600">{{ (isCache ? ' (Cached Product)' : '') }}</span>
      <h3 class="pl-5 text-xl font-extrabold mt-5">Bullet Point Features</h3>
      <ul class="list-disc pl-10 pt-5" v-for="(b, index) in bullet" :key="index">
        <li>{{ b }}</li>
      </ul>
      <h3 class="pl-5 text-xl font-extrabold mt-5">Rating count</h3>
      ...
      <h3 class="pl-5 text-xl font-extrabold mt-5">Average rating</h3>
      <div class="pl-5 pt-2 flex items-center">
        <svg class="w-4 h-4 text-yellow-300 me-1" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
            <path d="M20.924 7.625a1.523 1.523 0 0 0-1.238-1.044l-5.051-.734-2.259-4.577a1.534 1.534 0 0 0-2.752 0L7.365 5.847l-5.051.734A1.535 1.535 0 0 0 1.463 9.2l3.656 3.563-.863 5.031a1.532 1.532 0 0 0 2.226 1.616L11 17.033l4.518 2.375a1.534 1.534 0 0 0 2.226-1.617l-.863-5.03L20.537 9.2a1.523 1.523 0 0 0 .387-1.575Z"/>
        </svg>
      ...
      <h3 class="pl-5 text-xl font-extrabold mt-5">Average buy box price for the last 30, 90 and 180 days</h3>
      <ul class="list-disc pl-10 pt-5" v-for="(price, index) in averagePrices" :key="index">
        <li>{{ price.day }} days: {{ price.avgPrice.toFixed(2) }}$</li>
      </ul>
      <h3 class="pl-5 text-xl font-extrabold mt-5">Amazon in-stock rate for the last 30, 90, 180 days</h3>
...
<script setup>
  ...
  const isAsin = ref(false);
  const code = ref(''); // Code of product (ASIN)
  const pcode = ref('');
  const summary = ref('');
  ...
  onMounted(() => {
    ...
    const c = getParameterByName('search');
    if(type && type.includes('productCode')) {
      pcode.value = c;
      isAsin.value = false;
    }     
    else {
  ...
  async function submitCode(type, c) {
    if(!c) return;
    // Send the GET request to backend
    const response = await axios.get(process.env.VUE_APP_BACKEND_URL + 'products/' + type + '/' + (c ? c : '1'));
